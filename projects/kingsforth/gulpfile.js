...
gulp.task('compile-less', function () {
    var less_tasks = [];
    gulp.src(templatedir + 'src/assets/less/style-default.less')
        .pipe(less())
...
gulp.task('compress-js', function () {
    gulp.src(templatedir + 'src/assets/js/*.js')
        .pipe(uglify())
        .pipe(rename({ suffix: '.min' }))
        .pipe(gulp.dest(templatedir + 'src/assets/js/min'));
});
...
gulp.task('watch-less', ['compile-less'], function () {
    ...
    //watch js file
    gulp.watch(templatedir + 'src/assets/js/*.js', ['compress-js']);
